...
int io4_set_value(IO4 *io4, uint8_t value_mask) {
	...
	ipcon_mutex_lock(&io4->write_mutex);
	...
	ipcon_mutex_unlock(&io4->write_mutex);
...
int io4_get_value(IO4 *io4, uint8_t *ret_value_mask) {
	...
	io4->response.function_id = FUNCTION_GET_VALUE;
	io4->response.length = sizeof(GetValueReturn_);
	GetValue_ gv;
	...
	if(ipcon_device_expect_response(io4) != 0) {
		ipcon_mutex_unlock(&io4->write_mutex);
		return E_TIMEOUT;
	}

	GetValueReturn_ *gvr = (GetValueReturn_ *)io4->response.buffer;
...
int io4_get_configuration(IO4 *io4, uint8_t *ret_direction_mask, uint8_t *ret_value_mask) {
	...
	io4->response.function_id = FUNCTION_GET_CONFIGURATION;
	io4->response.length = sizeof(GetConfigurationReturn_);
	...
	GetConfigurationReturn_ *gcr = (GetConfigurationReturn_ *)io4->response.buffer;
...
int io4_get_debounce_period(IO4 *io4, uint32_t *ret_debounce) {
	...
	io4->response.function_id = FUNCTION_GET_DEBOUNCE_PERIOD;
	io4->response.length = sizeof(GetDebouncePeriodReturn_);
	...
	GetDebouncePeriodReturn_ *gdpr = (GetDebouncePeriodReturn_ *)io4->response.buffer;
...
int io4_get_interrupt(IO4 *io4, uint8_t *ret_interrupt_mask) {
	...
	io4->response.function_id = FUNCTION_GET_INTERRUPT;
	io4->response.length = sizeof(GetInterruptReturn_);
	GetInterrupt_ gi;
	...
	GetInterruptReturn_ *gir = (GetInterruptReturn_ *)io4->response.buffer;
...
static int io4_callback_wrapper_interrupt(IO4 *io4, const unsigned char *buffer) {
	InterruptCallback_ *ic = (InterruptCallback_ *)buffer;
	((interrupt_func_t)io4->registered_callbacks[ic->function_id])(ic->interrupt_mask, ic->value_mask);
	return sizeof(InterruptCallback_);
}

void io4_register_callback(IO4 *io4, uint8_t id, void *callback) {
    io4->registered_callbacks[id] = callback;
}
...
void io4_create(IO4 *io4, const char *uid) {
	...
	io4->callback_wrappers[IO4_CALLBACK_INTERRUPT] = io4_callback_wrapper_interrupt;
}
